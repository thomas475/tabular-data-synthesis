"""Rapidly Converging Gibbs algorithm (RACOG) implementation for data oversampling"""

# Authors: Arseniy Kustov <https://github.com/airysen>
...
class RACOG(BaseOverSampler):
    ...
    continuous_distribution: 'normal' or 'laplace'
        The distribution using for sampling (reconstruct) continuous variables
        after oversampling
    ...
    def __init__(self, sampling_strategy='auto', random_state=None, discretization='caim', categorical_features='auto',
                 burnin=100, lag=20, n_iter='auto', ignition_step='auto', root=0,
                 continuous_distribution='normal', only_sampled=False, alpha=0.6, L=0.5,
        ...
        elif continuous_distribution == 'laplace':
            self.f_sample = np.random.laplace
        else:
            raise WrongDistributionException("'continuous_distribution' must be 'normal' or 'laplace'")
        self.root = root
    ...
    def _fit_resample(self, X, y):
        ...
        for elem in self.sampling_strategy_:
            print('class', elem, ':', self.sampling_strategy_[elem])
        ...
        for class_sample, n_samples in self.sampling_strategy_.items():
            ...
            count = 0
            if n_iter == 'auto':
                n_iter = offset + np.ceil(n_samples / X_class.shape[0]) * lag
                # n_iter = offset + ((X_di.shape[0] - 1.6 * X_class.shape[0]) / X_class.shape[0]) * lag
            ...
            samples_per_gibbs = int((n_iter - offset) / lag)
