...
import numpy as np
import pandas as pd
from framework.generators.snsynth.preprocessors.data_transformer import BaseTransformer
...
class DPMinMaxTransformer(BaseTransformer):
    ...
    def fit(
        self, raw_data, discrete_columns=tuple(), continuous_columns_lower_upper=None
    ):
        ...
        if not continuous_columns_lower_upper:
            continuous_columns_lower_upper = {}
            for column_name in raw_data.columns:
                raw_column_data = raw_data[column_name]
                if not column_name in discrete_columns:
                    continuous_columns_lower_upper[column_name] = {}
                    continuous_columns_lower_upper[column_name][0] = raw_column_data.min()
                    continuous_columns_lower_upper[column_name][1] = raw_column_data.max()
    ...
    def _inverse_transform_continuous(
        ...
        scaler = column_transform_info.transform
        selected_normalized_value = column_data.iloc[:, 0]
