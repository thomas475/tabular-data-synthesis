...
class BaseTransformer(object):
    ...
    def _fit_discrete(self, column_name, raw_column_data):
        ...
        ohe.fit(pd.DataFrame(raw_column_data), [column_name])
    ...
    def _transform_discrete(self, column_transform_info, raw_column_data):
        ...
        return [ohe.transform(pd.DataFrame(raw_column_data).reset_index(drop=True))]
    ...
    def transform(self, raw_data):
        ...
        transformed_data = pd.concat([pd.DataFrame(column) for column in column_data_list], axis=1).astype(float)
        self._transformed_column_titles = transformed_data.columns

        return transformed_data.to_numpy().astype(float)
    ...
    def inverse_transform(self, data, sigmas=None):
        ...
        for column_transform_info in self._column_transform_info_list:
            ...
            column_data.columns = self._transformed_column_titles[st:st + dim]
